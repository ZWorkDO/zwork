  ...
  <div>
      ...
      <fieldset>
        <div>
          <span class="wt-select">
            <select id="freelancer_category">
              <option
                v-for="(stored_category, index) in stored_categories"
                :key="index"
                :value="stored_category.id"
              >{{stored_category.title}}</option>
            </select>
          </span>
        </div>
        <div class="form-group wt-btnarea">
          <a
            ...
            class="wt-btn-cat wt-btn"
            @click="addCategory"
          >{{trans('lang.add_categories')}}</a>
        </div>
      </fieldset>
    </div>
    <div class="wt-mycategories">
      <ul id="category_list" class="sortable list">
        <li
          v-for="(freelancer_category, index) in freelancer_categories"
          :key="index"
          v-if="freelancer_categories"
        ...
        >
          ...
          <span class="category-dynamic-html">{{freelancer_category.title}}</span>
          <span class="category-dynamic-field sss">
            <input
              type="hidden"
              v-bind:name="'categories['+index+'][id]'"
              :value="freelancer_category.id"
            />
          </span>
          <div class="wt-rightarea">
            <a
              href="javascript:void(0);"
              class="wt-deleteinfo delete-category"
              @click="removeStoredCategory(index)"
            >
        ...
        <li v-for="(category, index) in categories" :key="index+category.count">
          ...
          <span class="category-dynamic-field">
            <input
              type="hidden"
              v-bind:name="'categories['+[category.count]+'][id]'"
              :value="category.id"
            />
          </span>
          <div class="wt-rightarea">
            <a href="javascript:void(0);" class="wt-deleteinfo" @click="removeCategory(index)">
              <i class="lnr lnr-trash"></i>
            </a>
          </div>
        </li>
...
export default {
  ...
  methods: {
    ...
    getAllCategories() {
      let self = this;
      axios
        .get(APP_URL + "/get-freelancer-all-categories")
        .then(function(response) {
          self.stored_categories = response.data.categories;

          axios
            .get(APP_URL + "/freelancer/get-freelancer-categoties")
            .then(function(response) {
              self.freelancer_categories = response.data.freelancer_categories;

              if(self.freelancer_categories && self.freelancer_categories.length > 0){

                  if(self.stored_categories && self.stored_categories.length > 0){
                    for (let index = 0; index < self.stored_categories.length; index++) {
                        for (let index2 = 0; index2 < self.freelancer_categories.length; index2++) {
                            if(self.freelancer_categories[index2].id == self.stored_categories[index].id){
                                self.stored_categories.splice(index, 1);
                            }
                        }
                    }
                  }
    ...
    addCategory: function() {
      ...
      if (categoriesSelect.value === "") {
        ...
        var category_list_count = jQuery(".wt-btn-cat")
          .parents(".wt-categoryform")
          .next(".wt-mycategories")
          .find("ul#category_list li").length;
        ...
        if (categoriesSelect.options[categoriesSelect.selectedIndex]) {
          this.selected_category_text =
            categoriesSelect.options[categoriesSelect.selectedIndex].text;
          this.selected_category = document.getElementById(
            "freelancer_category"
          ).value;
          ...
          this.categories.push(
            Vue.util.extend(
              ...
              this.category,
              this.category.count++,
              (this.category.title = this.selected_category_text),
              (this.category.id = this.selected_category)
            )
          );
