<div class="wt-widget la-empinfo-holder">
    <div class="wt-companysdetails">
        <figure class="wt-companysimg">
            <img src="{{{ asset(Helper::getUserProfileProjectBanner($job->employer->id, 'small')) }}}" alt="{{ trans('lang.profile_img') }}">
        </figure>
        <div class="wt-companysinfo">
            ...
            <div class="wt-title">
                @if ($job->employer->user_verified === 1)
                    <a href="{{{ url('profile-project/'.$job->employer->slug) }}}"><i class="fa fa-check-circle"></i> {{ trans('lang.verified_company') }}</a>
                @endif
                <a href="{{{ url('profile-project/'.$job->employer->slug) }}}"><h2>{{{ Helper::getUserName($job->employer->id) }}}</h2></a>
            </div>
